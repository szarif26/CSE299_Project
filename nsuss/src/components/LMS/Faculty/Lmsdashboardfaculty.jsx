...
import classes from "./Lmsdashboardfaculty.module.css";

const Lmsdashboardfaculty = () => {
  const [courses, setCourses] = useState([]);
  useEffect(() => {
    getCourses();
  }, []);
  const username = JSON.parse(window.localStorage.getItem("user")).name;
  const facultyid = JSON.parse(window.localStorage.getItem("user")).userid;


  const getCourses = async () => {
    let result = await fetch(`http://localhost:4000/getcourse/${facultyid}`);
        ...
        <div className={classes.col}>
        {
            courses.map((courses, index) => (
              <ul key={courses.courseid_sec} >
                <Link to={"/coursepageFac/" + courses.courseid_sec} className={classes.nounderline} >
                  <button type="submit" className={classes.button}>
...
export default Lmsdashboardfaculty;
